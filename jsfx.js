(function(jsfx){
	...
	// Same as SoundsImmediate, but avoids locking the browser for too long
	...
	// in case you have a large amount of sounds to generate
	var variations = 5;
	jsfx.Sounds2 = function(library, ondone, onprogress){
		var audio  = {};
		var player = {};
		player._audio = audio;
		...
		map_object(library, function(_, name){
			...
			player[name] = function(){
				var set = audio[name];
				if(set.length !== 0){
					var elem = set[Math.random()*set.length|0];
		...
		function next(){
			if(toLoad.length == 0){
				ondone && ondone(library);
				return;
			}
			var name = toLoad.shift();
			audio[name].push(jsfx.Sound(library[name]));
			loaded++;
			onprogress && onprogress(name, loaded, total);
	...
	jsfx.NewGenerator = newGenerator;
...
(function(jsfx){
	var space = { f:[0, 0, 0, 0], w: [0, 0, 0], s:1, p: 1};

	var letterspec = {
		i: { f:[250, 2250, 3000, 3500], s:6, a:3, p: 0},
		j: { f:[5,  56,  0, 0], 		s:1, a: 3, p:0 },
		e: { f:[450, 2000, 2600, 3450], s:3, a:15, p: 0},
		E: { f:[600, 1800, 2600, 3500], s:3, a:3, p: 0},
		a: { f:[750, 1450, 2500, 3400], s:3, a:15, p: 0},
		A: { f:[700, 1200, 2750, 3300], s:5, a:15, p: 0},
		o: { f:[600, 1100, 2500, 3250], s:2, a:3, p: 0},
		O: { f:[400, 800, 2500, 3200],  s:12, a:6, p: 0},
		u: { f:[250, 700, 2350, 3250],  s:3, a:3, p: 0},
		y: { f:[300, 1750, 2200, 3250], s:5, a:3, p: 0},

		// table values based on TSS (http://heckmeck.de/demoscene/tiny-speech-synth-js/tss.js)
		w: { f:[ 3,  14,  0, 0], s:3, a: 1, p:0 },
		v: { f:[ 2,  20,  0, 0], s:3, a: 3, p:0 },
		T: { f:[ 2,  20,  0, 0], s:3, a: 5, p:0 },
		z: { f:[ 5,  28, 80, 0], s:3, a: 3, p:0 },
		Z: { f:[ 4,  30, 60, 0], s:3, a: 5, p:0 },
		b: { f:[ 4,   0,  0, 0], s:1, a: 2, p:0 },
		d: { f:[ 4,  40, 80, 0], s:1, a: 2, p:0 },
		m: { f:[ 4,  20,  0, 0], s:3, a: 2, p:0 },
		n: { f:[ 4,  40,  0, 0], s:3, a: 2, p:0 },
		r: { f:[ 3,  10, 20, 0], s:3, a: 3, p:0 },
		l: { f:[ 8,  20,  0, 0], s:3, a: 5, p:0 },
		g: { f:[ 2,  10, 26, 0], s:2, a: 1, p:0 },

		h: { f:[22,  26, 32, 0], s:1, a:10, p:0 },
		s: { f:[80, 110,  0, 0], s:3, a: 5, p:0 },
		S: { f:[20,  30,  0, 0], s:3, a:10, p:0 },
		p: { f:[ 4,  10, 20, 0], s:1, a: 2, p:1 },
		t: { f:[ 4,  20, 40, 0], s:1, a: 3, p:1 },
		k: { f:[20,  80,  0, 0], s:1, a: 3, p:1 }
	};

	var TAU = Math.PI * 2;

	jsfx.AlienAudio = function(sentence, letterspeed){
		letterspeed = letterspeed || 0.08;
		var samplesPerLetter = jsfx.SampleRate * letterspeed | 0;
		var totalSamples = 0;
		for(var i = 0; i < sentence.length; i++){
			var spec = letterspec[sentence[i]] || space;
			totalSamples += spec.s * samplesPerLetter;
			if(!spec.p){
				sample -= (samplesPerLetter / 3)|4;
			};
		}

		var block = jsfx._createFloatArray(totalSamples);

		var sample = 0;
		var freq = 220;
		var phase = 0;
		var phaseSpeed = freq * TAU / jsfx.SampleRate;
		...
		for(var i = 0; i < sentence.length; i++){
			...
			if(spec.p){
				var t = 1;
				for(var k = 0; k < samplesPerLetter*spec.s; k++){
					block[sample] = (Math.random() - 0.5) * spec.a * t;
					t *= 0.9995;
					sample++;
					phase += phaseSpeed;
					if(phase > TAU){ phase -= TAU; }
				...
				for(var k = 0; k < samplesPerLetter*spec.s; k++){
					block[sample] += (Math.random() - 0.5) * 0.1 +
						Math.cos(phase*a0) * 0.45 +
						Math.cos(phase*a1) * 0.25 +
						Math.cos(phase*a2) * 0.35 +
						Math.cos(phase*a3) * 0.55;

					block[sample] *= spec.a/10;

					sample++;
					phase += phaseSpeed;
					if(phase > TAU){ phase -= TAU; }
					a0 = (a0*0.995 + t0*0.005);
					a1 = (a1*0.995 + t1*0.005);
					a2 = (a2*0.995 + t2*0.005);
					a3 = (a3*0.995 + t3*0.005);
				}
				sample -= (samplesPerLetter / 3)|4;
			}
		}
		block = block.slice(0, sample);
		...
		return jsfx.CreateAudio(block);
	};

	jsfx.Alien = function(sentence, letterspeed){
		var audio = jsfx.AlienAudio(sentence, letterspeed);
		return function(){
	...
	function letterfn(f){
		return "sample = " +
			"Math.cos(phase*" + f[0]/f[0] + ") * 0.55 + " +
			"Math.cos(phase*" + f[1]/f[0] + ") * 0.35 + " +
			"Math.cos(phase*" + f[2]/f[0] + ") * 0.45 + " +
			"Math.cos(phase*" + f[3]/f[0] + ") * 0.65;";
	};
	jsfx.G.alien = jsfx.NewGenerator(letterfn([750, 1450, 2500, 3400]));
})(this.jsfx);
